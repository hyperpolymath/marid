...
│       └── include/
│           └── marid.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── marid.h       # Generated from Idris2 ABI
│
...
----
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/marid.h
----
...
int main() {
    void* handle = marid_init();
    if (!handle) return 1;

    int result = marid_process(handle, 42);
    if (result != 0) {
        const char* err = marid_last_error();
...
[source,bash]
----
gcc -o example example.c -lmarid -L./zig-out/lib
----
...
#[link(name = "marid")]
extern "C" {
    fn marid_init() -> *mut std::ffi::c_void;
    fn marid_free(handle: *mut std::ffi::c_void);
    fn marid_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = marid_init();
        assert!(!handle.is_null());

        let result = marid_process(handle, 42);
        assert_eq!(result, 0);
...
[source,julia]
----
const libmarid = "libmarid"

function init()
    handle = ccall((:marid_init, libmarid), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:marid_process, libmarid), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:marid_free, libmarid), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/marid.h
   ```
