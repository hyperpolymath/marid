...
# entirely, so nobody noticed that .github/settings.yml shipped `name: "marid"`
...
# an instantiation. The template submitted the literal `marid` as its own
...
# is set to `owner/marid` and matches — but wrong anywhere the
...
# deliberate placeholder files as a failure. A clone into `marid-2`,
